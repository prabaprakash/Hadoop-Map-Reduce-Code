...
public class IngridientSearch {
	...
	public static class TokenizerMapper
		...
		public void map(Object key, Text value, Context context)
			...
			if (ia.length == 5) {
				if (roo.ingredients.contains("mango")) {
					one.set(p);
					word.set(roo.name);
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf, "IngridientSearch");

		job.setJarByClass(IngridientSearch.class);
		...
		FileOutputFormat.setOutputPath(job, new Path(
				"hdfs://127.0.0.1:9000/IngridentSearch"));
